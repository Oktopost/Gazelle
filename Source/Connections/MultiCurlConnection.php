...
{
	// curl_multi_select() only accepts non-negative timeouts; a negative $timeout means
	// "wait indefinitely" to callers, so it is clamped to this effectively-infinite value.
	private const MAX_SELECT_TIMEOUT = 2147483.0;
	...
	private function handleResult(mixed $handle): MultiResult
	{
		...
		catch (RequestException|ResponseException $re)
		{
	...
	public function toResult(\CurlHandle $handle, IRequestParams $request): Response
	{
		...
		if ($request->getParseResponseForErrors())
		{
	...
	public function next(float $timeout = 0.1): ?MultiResult
	{
		...
		if (!$info)
		{
			curl_multi_select($multiHandle, $timeout < 0 ? self::MAX_SELECT_TIMEOUT : $timeout);
			$info = curl_multi_info_read($this->multiHandle);
		}
